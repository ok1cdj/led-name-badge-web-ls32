export const FONT_11X44 = new Uint8Array([
...
// Build lookup using last occurrence, matching Python's dict-overwrite behavior.
// Space ' ' appears multiple times; only the last one (all-zero bytes) is correct.
const charOffsets: Record<string, number> = {};
for (let i = 0; i < CHARMAP.length; i++) {
  charOffsets[CHARMAP[i]] = i;
}

export const ICONS: Record<string, Uint8Array> = {
  ball:  new Uint8Array([0x00,0x00,0x3c,0x7e,0xff,0xff,0xff,0xff,0x7e,0x3c,0x00]),
  happy: new Uint8Array([0x00,0x00,0x3c,0x42,0xa5,0x81,0xa5,0x99,0x42,0x3c,0x00]),
  heart: new Uint8Array([0x00,0x00,0x6c,0x92,0x82,0x82,0x44,0x28,0x10,0x00,0x00]),
  HEART: new Uint8Array([0x00,0x00,0x6c,0xfe,0xfe,0xfe,0x7c,0x38,0x10,0x00,0x00]),
};

export function getCharBitmap(ch: string): Uint8Array {
  const idx = charOffsets[ch];
  if (idx === undefined) {
    return new Uint8Array(11);
  }
  return FONT_11X44.slice(idx * 11, idx * 11 + 11);
}
